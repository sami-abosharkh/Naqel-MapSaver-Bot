...
def ShipmentsDetails(aNum):
    ...
    for a in range(3):
        ...
        if shipmentPin == 0:
            ...
            for a in range(3):
                if len(driver.find_elements(by=AppiumBy.XPATH, value="(//android.view.View[@content-desc='" + shipmentNum + ". '])")) > 0:
...
def ListScroll(): 
    ...
    actions.w3c_actions.pointer_action.move_to_location(300, 1300)
    actions.w3c_actions.pointer_action.pointer_down()
    actions.w3c_actions.pointer_action.move_to_location(300, 950)
    actions.perform()

def MapScroll(): 
    actions = ActionChains(driver)
    actions.w3c_actions = ActionBuilder(driver, mouse=PointerInput(interaction.POINTER_TOUCH, "Map"))
    actions.w3c_actions.pointer_action.move_to_location(300, 1150)
    actions.w3c_actions.pointer_action.pointer_down()
    actions.w3c_actions.pointer_action.move_to_location(300, 1300)
    actions.perform()
